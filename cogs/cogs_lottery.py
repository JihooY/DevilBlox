...
class LotteryCog(commands.Cog):
    ...
    async def open_lottery_ticket(self, interaction: discord.Interaction, 추첨번호: str = ""):
        ...
        entry = await self.repos.lottery.mark_opened(interaction.guild.id, event["_id"], interaction.user.id)
        shapes = entry.get("shapes") or []
        result = format_shapes(shapes)
        if entry.get("is_winner"):
            embed = success_embed("당첨!", f"`{result}`\n같은 도형 3개 연속입니다.")
        else:
            embed = error_embed("아쉽게도 꽝", f"`{result}`\n같은 도형 3개 연속이 나오지 않았습니다.")
